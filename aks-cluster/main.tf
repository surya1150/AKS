...
resource "azurerm_kubernetes_cluster" "main" {
  ...
  # coalesce(var.location, data.azurerm_resource_group.main.location)
  ...
  private_cluster_public_fqdn_enabled = var.private_cluster_public_fqdn_enabled #private_cluster_public_fqdn_enabled is a setting that controls whether a Kubernetes cluster running in a private network is also accessible via a publicly resolvable Fully Qualified Domain Name (FQDN).
  private_dns_zone_id                 = var.private_dns_zone_id
  public_network_access_enabled       = var.public_network_access_enabled # default to true, Set of authorized IP ranges in  api_server_access_profile to allow access to API server, e.g. ["198.51.100.0/24"].
  role_based_access_control_enabled   = var.role_based_access_control_enabled
  sku_tier                            = var.sku_tier #sku_tier refers to the tier or pricing level of a resource in Microsoft Azure.
  tags                                = var.tags
